...
import { ROUNDS, VERIFIED_ORG, SUBMITABLE_ROUNDS, MEDIA, POST_PROJECTS, NEW_PROJECT_DETAIL, LOCATIONS, NOT_VERIFIED_ORG } from "./mocks";
...
describe('Organization creates project', () => {
    ...
    it('verified organization creates Project', () => {
        ...
        cy.get('#social-causes').focus().type('Social');
        ...
        cy.get('[name=title]').focus().type('Test project');
        cy.get('[name=website]').focus().type('http://test.com');
        cy.get('[role=textbox]').focus().type('this is a test paragraph {enter}');

        cy.contains('button', 'B').click();
        cy.get('[role=textbox]').focus().type('this test is bold {enter}');

        cy.contains('button', 'i').click();
        cy.get('[role=textbox]').focus().type('this text is itallic and bold {enter}');

        cy.contains('button', 'B').click();
        cy.get('[role=textbox]').focus().type('this text is only itallic {enter}');

        cy.contains('button', 'i').click();
        cy.contains('button', 'u').click();
        cy.get('[role=textbox]').focus().type('this text has underline {enter}');

        cy.contains('button', 'i').click();
        cy.contains('button', 'B').click();
        cy.get('[role=textbox]').focus().type('this text has all at the same time {enter}');
        ...
        cy.get('.tiptap.ProseMirror').invoke('html').then(html => {
            ...
            expect(html).to.include('<strong class="font-bold"><em class="italic">this text is itallic and bold </em></strong>');
            ...
            expect(html).to.include('<strong class="font-bold"><em class="italic"><u class="underline">this text has all at the same time </u></em></strong>');
        });
    ...
    it('user selects location', () => {
        ...
        cy.get('[data-testid="search-dropdown"]').click().type('tokyo');
